...
public class Drive : MonoBehaviour
{
    ...
    string[] aiNames = { "John", "Sean", "Jotty", "Kabaya" };
    ...
    void Start()
    {
        ...
        playerNameObj.GetComponent<NameUIController1>().target = rb.gameObject.transform;
        if (this.GetComponent<AIController>().enabled)
        {
            playerNameObj.GetComponent<TextMeshProUGUI>().text = aiNames[Random.Range(0, aiNames.Length)];

        }
        else {
            playerNameObj.GetComponent<TextMeshProUGUI>().text = "Human";
        }
